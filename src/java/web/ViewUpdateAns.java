 ...
 * @author USER-1
 */
public class ViewUpdateAns extends HttpServlet {
...
if(request.getSession().getAttribute("lId")!=null && request.getSession().getAttribute("role").equals("expert")){ 
        ...
        c.add(Restrictions.eq("exIdfk",request.getSession().getAttribute("lId") ));
        List<Answers> data=c.list();
        request.setAttribute("ansdata", data);
        
        RequestDispatcher rd=request.getRequestDispatcher("viewupdateans.jsp");
